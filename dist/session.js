...
var Session = class {
	...
	*/
	async ensureClient() {
		...
		throw new Error("Session is not attached to a live in-browser sandbox host.");
	}
	get client() {
	...
	*/
	async downloadFile(src, dst, opts) {
		...
		const { createWriteStream } = await import(
			/* webpackIgnore: true */
			/* @vite-ignore */
			"fs"
);
		...
		await pipeline(Readable.fromWeb(stream), createWriteStream(dstPath), { signal: opts?.signal });
		return dstPath;
	}
	/**
	...
	*/
	async writeFiles(files, opts) {
		await (await this.ensureClient()).writeFiles({
			sessionId: this.session.id,
