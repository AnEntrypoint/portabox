const require_browser_primitives = require('./utils/browser-primitives.cjs');
...
var Sandbox = class {
	...
	*/
	static async list() {
		throw new require_webix_client.NotSupportedError("Sandbox.list is unavailable in the self-contained backend: there is no remote account registry. Each in-browser sandbox exists only in the page that created it.");
	}
	/**
	...
	*/
	static async create(params) {
		if (params?.source) throw new require_webix_client.NotSupportedError("Sandbox sources (git/tarball/snapshot URLs) are not supported by the in-browser backend; seed files with writeFiles() instead.");
		const runtime = params && "runtime" in params ? params.runtime : void 0;
		const client = new require_webix_client.WebixApiClient({
	...
	*/
	static async fork() {
		throw new require_webix_client.NotSupportedError("Sandbox.fork is unavailable in the self-contained in-browser backend.");
	}
	static async get() {
		throw new require_webix_client.NotSupportedError("Sandbox.get is unavailable in the self-contained in-browser backend; keep the Sandbox returned by create().");
	}
	static async getOrCreate() {
		throw new require_webix_client.NotSupportedError("Sandbox.getOrCreate is unavailable in the self-contained in-browser backend; use create().");
	}
	/**
	...
	*/
	async waitForStopAndResume(signal) {
		...
		while (status === "stopping" || status === "snapshotting") {
			await require_browser_primitives.sleepWithSignal(pollingInterval, signal);
