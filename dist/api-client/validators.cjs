...
const Sandbox = zod.z.object({
	...
	keepLastSnapshots: zod.z.object({
		count: zod.z.number(),
		expiration: zod.z.number().optional(),
		deleteEvicted: zod.z.boolean().optional()
	}).optional()
});
...
const SandboxAndSessionResponse = zod.z.object({
	...
	resumed: zod.z.boolean().optional()
});
const SandboxesPaginationResponse = zod.z.object({
	sandboxes: zod.z.array(Sandbox),
	pagination: CursorPagination
});
const UpdateSandboxResponse = zod.z.object({
	sandbox: Sandbox,
	routes: zod.z.array(SandboxRoute).optional()
});
