	...
	*/
	...
	async readdir(path, options) {
		...
		if (options?.withFileTypes) {
			const result$1 = await this.sandbox.runCommand("ls", ["-la", path], { signal: options?.signal });
			...
			const lines = (await result$1.stdout()).trim().split("\n").filter(Boolean);
			const dirents = [];
			for (const line of lines) {
				if (/^total\s+\d+/.test(line)) continue;
				const m = line.match(/^([-dlbcps])\S*\s+\S+\s+\S+\s+\S+\s+\S+\s+\S+\s+\S+\s+\S+\s+(.+)$/);
				if (!m) continue;
				const typeChar = m[1];
				let name = m[2];
